...
struct StatusBarIcon {
    ...
    static func icon(for state: QuotaState, sheepTint: NSColor? = nil) -> Result {
        switch state {
        case .loading, .error:
        ...
        case .loaded(let data):
            ...
            if let tint = sheepTint {
                return Result(image: renderIcon(suffix: nil, color: nil, sheepTint: tint))
            }
